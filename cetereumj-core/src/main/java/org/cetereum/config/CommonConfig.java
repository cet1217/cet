...
import org.cetereum.datasource.leveldb.LevelDbDataSource;
...
import org.cetereum.validator.*;
import org.cetereum.vm.DataWord;
import org.cetereum.vm.program.ProgramPrecompile;
...
public class CommonConfig {
     ...
     *
     * <b>Note:</b> is eligible as a public node provider, like in {@link Cet63};
    ...
    public void fastSyncCleanUp() {
        ...
        CetereumListener.SyncState syncStage = CetereumListener.SyncState.values()[fastsyncStageBytes[0]];

        if (!systemProperties().isFastSyncEnabled() || syncStage == CetereumListener.SyncState.UNSECURE) {
    ...
    @Bean(name = "CetereumListener")
    public CompositeCetereumListener cetereumListener() {
        return new CompositeCetereumListener();
    }
    ...
    public BlockHeaderValidator headerValidator() {
        ...
        List<BlockHeaderRule> rules = new ArrayList<>(asList(
                ...
                CetashRule.createRegular(systemProperties(), cetereumListener()),
