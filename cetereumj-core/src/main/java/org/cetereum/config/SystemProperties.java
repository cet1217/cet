...
import org.cetereum.config.blockchain.OlympicConfig;
import org.cetereum.config.net.*;
import org.cetereum.core.Genesis;
import org.cetereum.core.genesis.GenesisConfig;
import org.cetereum.core.genesis.GenesisJson;
import org.cetereum.core.genesis.GenesisLoader;
import org.cetereum.crypto.ECKey;
import org.cetereum.net.p2p.P2pHandler;
import org.cetereum.net.rlpx.MessageCodec;
import org.cetereum.net.rlpx.Node;
import org.cetereum.util.BuildInfo;
import org.cetereum.util.ByteUtil;
import org.cetereum.util.Utils;
import org.cetereum.validator.BlockCustomHashRule;
...
import java.lang.reflect.Mcetod;
...
/**
 * Utility class to retrieve property values from the cetereumj.conf files
 *
 ...
 * - resource cetereumj.conf : normally used as a reference config with default values
 *          and shouldn't be changed
 * - system property : each config entry might be altered via -D VM option
 * - [user dir]/config/cetereumj.conf
 * - config specified with the -Dcetereumj.conf.file=[file.conf] VM option
 * - CLI options
 *
...
public class SystemProperties {
     ...
     * be used
     * This mcetod is mainly used for testing purposes
    ...
    /**
     * Marks config accessor mcetods which need to be called (for value validation)
     * upon config creation or modification
     */
    @Target(ElementType.MCETOD)
    ...
    public SystemProperties(Config apiConfig, ClassLoader classLoader) {
        try {
            ...
            Config referenceConfig = ConfigFactory.parseResources("cetereumj.conf");
            logger.info("Config (" + (referenceConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): default properties from resource 'cetereumj.conf'");
            String res = System.getProperty("cetereumj.conf.res");
            Config cmdLineConfigRes = mergeConfigs(res, ConfigFactory::parseResources);
            logger.info("Config (" + (cmdLineConfigRes.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dcetereumj.conf.res resource(s) '" + res + "'");
            ...
            File userDirFile = new File(System.getProperty("user.dir"), "/config/cetereumj.conf");
            ...
            Config testConfig = ConfigFactory.parseResources("test-cetereumj.conf");
            logger.info("Config (" + (testConfig.entrySet().size() > 0 ? " yes " : " no  ") + "): test properties from resource 'test-cetereumj.conf'");
            ...
            String file = System.getProperty("cetereumj.conf.file");
            Config cmdLineConfigFile = mergeConfigs(file, s -> ConfigFactory.parseFile(new File(s)));
            logger.info("Config (" + (cmdLineConfigFile.entrySet().size() > 0 ? " yes " : " no  ") + "): user properties from -Dcetereumj.conf.file file(s) '" + file + "'");
    ...
    private void validateConfig() {
        for (Mcetod mcetod : getClass().getMcetods()) {
            try {
                if (mcetod.isAnnotationPresent(ValidateMe.class)) {
                    mcetod.invoke(this);
                }
            } catch (Exception e) {
                throw new RuntimeException("Error validating config mcetod: " + mcetod, e);
            }
        }
    }
            ...
            if (genesisJson.getConfig() != null && genesisJson.getConfig().isCustomConfig()) {
                ...
                if (config.hasPath("blockchain.config.name")) {
                    ...
                    try {
                        ...
                        throw new RuntimeException("The class specified via blockchain.config.class '" + className + "' is not instance of org.cetereum.config.BlockchainForkConfig", e);
    ...
    public String cetashDir() {
        return config.hasPath("cetash.dir") ? config.getString("cetash.dir") : databaseDir();
    }
    ...
    @ValidateMe
    public String getCetashMode() {
        return config.getString("sync.cetash");
    }
    ...
    /**
     * Mcetod used in StandaloneBlockchain.
     */
