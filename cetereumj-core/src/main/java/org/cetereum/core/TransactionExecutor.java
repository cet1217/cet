...
import static org.cetereum.vm.VMUtils.zipAndEncode;
...
public class TransactionExecutor {
    ...
    private final CetereumListener listener;
    ...
    public TransactionExecutor(Transaction tx, byte[] coinbase, Repository track, BlockStore blockStore,
        ...
        this(tx, coinbase, track, blockStore, programInvokeFactory, currentBlock, new CetereumListenerAdapter(), 0, VMHook.EMPTY);
    }

    public TransactionExecutor(Transaction tx, byte[] coinbase, Repository track, BlockStore blockStore,
                               ProgramInvokeFactory programInvokeFactory, Block currentBlock,
                               CetereumListener listener, long gasUsedInTheBlock) {
        this(tx, coinbase,track, blockStore, programInvokeFactory, currentBlock, listener, gasUsedInTheBlock, VMHook.EMPTY);
    }

    public TransactionExecutor(Transaction tx, byte[] coinbase, Repository track, BlockStore blockStore,
                               ProgramInvokeFactory programInvokeFactory, Block currentBlock,
                               CetereumListener listener, long gasUsedInTheBlock, VMHook vmHook) {
...
//            https://github.com/cetereum/cpp-cetereum/blob/develop/libcetereum/Executive.cpp#L241
            rollback();
            m_endGas = BigInteger.ZERO;
