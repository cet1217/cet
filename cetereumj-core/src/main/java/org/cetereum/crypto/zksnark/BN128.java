 ...
 */
package org.cetereum.crypto.zksnark;
 ...
 * use {@link #toCetNotation()} to convert Jacobian coords to Cetereum encoding <br/>
 *
...
public abstract class BN128<T extends Field<T>> {
    ...
    /**
     * Point at infinity in Cetereum notation: should return (0; 0; 0),
     * {@link #isZero()} mcetod called for that point, also, returns {@code true}
     */
     ...
     */
    public BN128<T> toCetNotation() {
        BN128<T> affine = toAffine();

        // affine zero is (0; 1; 0), convert to Cetereum zero: (0; 0; 0)
        if (affine.isZero()) {
            return zero();
        } else {
            return affine;
    ...
    protected boolean isValid() {
        ...
        // check whceter coordinates belongs to the Field
        if (!x.isValid() || !y.isValid() || !z.isValid()) {
            return false;
        }

        // check whceter point is on the curve
        if (!isOnCurve()) {
            return false;
        }
