...
import static org.cetereum.crypto.zksnark.Params.B_Fp;
...
public class BN128Fp extends BN128<Fp> {
    ...
    /**
     * Checks whceter x and y belong to Fp,
     * then checks whceter point with (x; y) coordinates lays on the curve.
     *
    ...
    public static BN128<Fp> create(byte[] xx, byte[] yy) {
        ...
        // check whceter point is a valid one
        if (p.isValid()) {
            return p;
        } else {
            return null;
