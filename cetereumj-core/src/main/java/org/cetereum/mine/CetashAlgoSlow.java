...
import static org.cetereum.crypto.HashUtil.sha512;
import static org.cetereum.util.ByteUtil.*;
import static org.spongycastle.util.Arrays.reverse;

/**
 * The Cetash algorithm described in https://github.com/cetereum/wiki/wiki/Cetash
 *
 * This is the non-optimized Cetash implementation. It is left here for reference only
 * since the non-optimized version is slightly better for understanding the Cetash algorithm
 *
 * Created by Anton Nashatyrev on 27.11.2015.
 * @deprecated Use a faster version {@link CetashAlgo}, this class is for reference only
 */
public class CetashAlgoSlow {
    CetashParams params;

    public CetashAlgoSlow() {
        this(new CetashParams());
    }

    public CetashAlgoSlow(CetashParams params) {
        this.params = params;
    }

    public CetashParams getParams() {
        return params;
    }
