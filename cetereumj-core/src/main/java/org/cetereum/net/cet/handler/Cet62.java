...
import static org.cetereum.datasource.MemSizeEstimator.ByteArrayEstimator;
import static org.cetereum.net.cet.CetVersion.V62;
import static org.cetereum.net.message.ReasonCode.USELESS_PEER;
import static org.cetereum.sync.PeerState.*;
import static org.cetereum.sync.PeerState.BLOCK_RETRIEVING;
import static org.cetereum.util.Utils.longToTimePeriod;
...
@Component("Cet62")
@Scope("prototype")
public class Cet62 extends CetHandler {
    ...
    protected CetState cetState = CetState.INIT;
    ...
    private static final CetVersion version = V62;

    public Cet62() {
        this(version);
    }

    Cet62(final CetVersion version) {
        super(version);
    }

    @Autowired
    public Cet62(final SystemProperties config, final Blockchain blockchain,
                 final BlockStore blockStore, final CompositeCetereumListener cetereumListener) {
        this(version, config, blockchain, blockStore, cetereumListener);
    }

    Cet62(final CetVersion version, final SystemProperties config,
    ...
    public synchronized ListenableFuture<List<BlockHeader>> sendGetBlockHeaders(long blockNumber, int maxBlocksAsk, boolean reverse) {
        ...
        if (cetState == CetState.STATUS_SUCCEEDED && peerState != IDLE) return null;
        ...
        try {
            ...
            if (!Arrays.equals(msg.getGenesisHash(), config.getGenesis().getHash())) {
                if (!peerDiscoveryMode) {
                    loggerNet.debug("Removing CetHandler for {} due to protocol incompatibility", ctx.channel().remoteAddress());
                }
                cetState = CetState.STATUS_FAILED;
                disconnect(ReasonCode.INCOMPATIBLE_PROTOCOL);
                ctx.pipeline().remove(this); // Peer is not compatible for the 'cet' sub-protocol
                return;
            }
            ...
            channel.getNodeStatistics().cetHandshake(msg);
            cetereumListener.onCetStatusUpdated(channel, msg);
            ...
            loggerNet.debug("CetHandler already removed");
        }
    }
    ...
    protected synchronized void processBlockHeaders(BlockHeadersMessage msg) {
        ...
        if (cetState == CetState.STATUS_SENT || cetState == CetState.HASH_CONSTRAINTS_CHECK)
    ...
    protected synchronized void sendNextHeaderRequest() {
        ...
        if (cetState == CetState.INIT) return;
    ...
    protected synchronized void processInitHeaders(List<BlockHeader> received) {
        ...
        if (cetState == CetState.STATUS_SENT) {
            ...
            cetState = CetState.HASH_CONSTRAINTS_CHECK;
        ...
        if (validatorMap.isEmpty()) {
            cetState = CetState.STATUS_SUCCEEDED;
    ...
    private void updateTotalDifficulty(BigInteger totalDiff) {
        channel.getNodeStatistics().setCetTotalDifficulty(totalDiff);
        this.totalDifficulty = totalDiff;
    }

    @Override
    public BigInteger getTotalDifficulty() {
        return totalDifficulty != null ? totalDifficulty : channel.getNodeStatistics().getCetTotalDifficulty();
    }
    ...
    public boolean hasStatusPassed() {
        return cetState.ordinal() > CetState.HASH_CONSTRAINTS_CHECK.ordinal();
    }

    @Override
    public boolean hasStatusSucceeded() {
        return cetState == CetState.STATUS_SUCCEEDED;
    }
