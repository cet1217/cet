...
/**
 * A list of commands for the Cetereum network protocol.
 * <br>
 * The codes for these commands are the first byte in every packet.
 *
 * @see <a href="https://github.com/cetereum/wiki/wiki/Cetereum-Wire-Protocol">
 * https://github.com/cetereum/wiki/wiki/Cetereum-Wire-Protocol</a>
 */
public enum CetMessageCodes {
     ...
     *
     * Inform a peer of it's current cetereum state. This message should be
     * send after the initial handshake and prior to any cetereum related messages.
     */
    STATUS(0x00),
     ...
     * are transactions in the format described in the main Cetereum specification.
     */
    TRANSACTIONS(0x02),
     ...
     * block headers in the format described in the main Cetereum specification,
     ...
     * in the format described in the main Cetereum specification, previously asked for in a GetBlockBodies message.
     * This may validly contain no block headers
     * if no block headers were able to be returned for the GetBlockHeaders query.
     */
    BLOCK_BODIES(0x06),
     ...
     * in the main Cetereum specification.
     */
    NEW_BLOCK(0x07),
    ...
    private static final Map<CetVersion, Map<Integer, CetMessageCodes>> intToTypeMap = new HashMap<>();
    private static final Map<CetVersion, CetMessageCodes[]> versionToValuesMap = new HashMap<>();

    static {

        versionToValuesMap.put(V62, new CetMessageCodes[]{
                STATUS,
                NEW_BLOCK_HASHES,
                TRANSACTIONS,
                GET_BLOCK_HEADERS,
        ...
        versionToValuesMap.put(V63, new CetMessageCodes[]{
                STATUS,
                NEW_BLOCK_HASHES,
                TRANSACTIONS,
                GET_BLOCK_HEADERS,
        ...
        for (CetVersion v : CetVersion.values()) {
            Map<Integer, CetMessageCodes> map = new HashMap<>();
            intToTypeMap.put(v, map);
            for (CetMessageCodes code : values(v)) {
                map.put(code.cmd, code);
            }
        }
    }

    private CetMessageCodes(int cmd) {
        this.cmd = cmd;
    }

    public static CetMessageCodes[] values(CetVersion v) {
        return versionToValuesMap.get(v);
    }

    public static int maxCode(CetVersion v) {

        int max = 0;
        for (CetMessageCodes cd : versionToValuesMap.get(v))
            if (max < cd.asByte())
                max = cd.asByte();
    ...
    public static CetMessageCodes fromByte(byte i, CetVersion v) {
        Map<Integer, CetMessageCodes> map = intToTypeMap.get(v);
        return map.get((int) i);
    }

    public static boolean inRange(byte code, CetVersion v) {
        CetMessageCodes[] codes = values(v);
