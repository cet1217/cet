...
import org.cetereum.net.server.CetereumChannelInitializer;
...
public class PeerClient {
    ...
    public PeerClient() {
        workerGroup = new NioEventLoopGroup(0, new ThreadFactory() {
            ...
            public Thread newThread(Runnable r) {
                return new Thread(r, "CetJClientWorker-" + cnt.getAndIncrement());
            }
        });
    }
    ...
    public ChannelFuture connectAsync(String host, int port, String remoteId, boolean discoveryMode) {
        cetereumListener.trace("Connecting to: " + host + ":" + port);

        CetereumChannelInitializer cetereumChannelInitializer = ctx.getBean(CetereumChannelInitializer.class, remoteId);
        cetereumChannelInitializer.setPeerDiscoveryMode(discoveryMode);
        ...
        b.handler(cetereumChannelInitializer);
