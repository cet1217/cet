...
import static org.cetereum.net.rlpx.FrameCodec.Frame;
...
//                    loggerNet.warn("No initial frame received for context-id: " + frame.contextId + ". Discarding this frame as invalid.");
    ...
    private Message decodeMessage(ChannelHandlerContext ctx, List<Frame> frames) throws IOException {
        ...
        cetereumListener.onRecvMessage(channel, msg);
    ...
    private byte getCode(Enum msgCommand){
        ...
        if (msgCommand instanceof CetMessageCodes){
            code = messageCodesResolver.withCetOffset(((CetMessageCodes) msgCommand).asByte());
        }
    ...
    private Message createMessage(byte code, byte[] payload) {
        ...
        resolved = messageCodesResolver.resolveCet(code);
        if (cetMessageFactory != null && CetMessageCodes.inRange(resolved, cetVersion)) {
            return cetMessageFactory.create(resolved, payload);
        }
    ...
    public void setCetVersion(CetVersion cetVersion) {
        this.cetVersion = cetVersion;
    }
    ...
    public void setCetMessageFactory(MessageFactory cetMessageFactory) {
        this.cetMessageFactory = cetMessageFactory;
    }
