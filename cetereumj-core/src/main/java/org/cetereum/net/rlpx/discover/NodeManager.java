...
import org.cetereum.db.PeerSource;
...
public class NodeManager implements Consumer<DiscoveryEvent>{
    ...
    @Autowired
    public NodeManager(SystemProperties config, CetereumListener cetereumListener,
    ...
    public synchronized NodeHandler getNodeHandler(Node n) {
        ...
        if (ret == null) {
            ...
            if (!n.isDiscoveryNode() && !n.getHexId().equals(homeNode.getHexId())) {
                cetereumListener.onNodeDiscovered(ret.getNode());
            }
    ...
    /**
     * Checks whceter peers with such InetSocketAddress has penalize disconnect record
    ...
    public List<NodeHandler> getNodes(
        ...
        filtered.sort((o1, o2) -> o2.getNodeStatistics().getCetTotalDifficulty().compareTo(
                o1.getNodeStatistics().getCetTotalDifficulty()));
