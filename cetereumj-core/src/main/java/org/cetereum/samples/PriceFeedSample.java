 ...
 *
 * As a side effect this sample shows how Java wrappers for Cetereum contracts can be
...
public class PriceFeedSample extends BasicSample {
    ...
    /**
     * Base class for a Cetereum Contract wrapper
     ...
     */
    abstract class CetereumContract {
         ...
         */
        protected CetereumContract(String contractAddr) {
         ...
         *  by their names only
         */
        public CetereumContract(String contractAddr, String contractABI) {
            this.contractAddr = contractAddr;
            this.contractFromABI = new CallTransaction.Contract(contractABI);
        }

        /**
         *  The main mcetod of this demo which illustrates how to call a constant function.
        ...
        protected Object[] callFunction(String name, String[] inParamTypes, String[] outParamTypes, Object ... args) {
            ...
            ProgramResult result = cetereum.callConstantFunction(contractAddr, function, args);
            return function.decodeResult(result.getHReturn());
        }

        /**
         *  Use this mcetod if the contract ABI was passed
         */
        protected Object[] callFunction(String functionName, Object ... args) {
            if (contractFromABI == null) {
                throw new RuntimeException("The contract JSON ABI should be passed to constructor to use this mcetod");
            }
        ...
        public boolean isExist() {
            return !contractAddr.equals(zeroAddr) && cetereum.getRepository().isExist(Hex.decode(contractAddr));
        }
    }
     ...
     * https://live.ceter.camp/account/985509582b2c38010bfaa3c8d2be60022d3d00da
     */
    class NameRegContract extends CetereumContract {
     ...
     *
     * Contract sources, live state and many more here:
     * https://live.ceter.camp/account/1194e966965418c7d73a42cceeb254d875860356
     */
    class PriceFeedContract extends CetereumContract {
...
//            worker();
    ...
    /**
     * The mcetod retrieves the information from the PriceFeed contract once in a minute and prints
     * the result in log.
     */
    private void worker() throws Exception{
        ...
        String priceFeedAddress = Hex.toHexString(nameRegContract.addressOf("ceter-camp/price-feed"));
        ...
        String[] tickers = {"BTC_CET", "USDT_BTC", "USDT_CET"};
        while(true) {
