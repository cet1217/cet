...
import org.cetereum.mine.CetashListener;
 ...
 * CetereumJ peers in a single JVM. For this two Spring ApplicationContext's are created which
...
public class PrivateMinerSample {
     ...
     */
    static class MinerNode extends BasicSample implements CetashListener {
        public MinerNode() {
        ...
        @Override
        public void onDatasetUpdate(CetashListener.DatasetStatus minerStatus) {
            logger.info("Miner status updated: {}", minerStatus);
            if (minerStatus.equals(CetashListener.DatasetStatus.FULL_DATASET_GENERATE_START)) {
            ...
            for (int i = cetereum.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 20000; i++, j++) {
                {
                    Transaction tx = new Transaction(ByteUtil.intToBytesNoLeadZeroes(i),
                            ...
                            receiverAddr, new byte[]{77}, new byte[0], cetereum.getChainIdForNextBlock());
                    tx.sign(senderKey);
    ...
    /**
     *  Creating two CetereumJ instances with different config classes
     */
    public static void main(String[] args) throws Exception {
        ...
        BasicSample.sLogger.info("Starting CettereumJ miner instance!");
        CetereumFactory.createCetereum(MinerConfig.class);

        BasicSample.sLogger.info("Starting CettereumJ regular instance!");
