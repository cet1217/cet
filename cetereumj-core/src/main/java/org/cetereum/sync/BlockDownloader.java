                    ...
                    for (Iterator<SyncQueueIfc.HeadersRequest> it = hReq.iterator(); it.hasNext();) {
                        ...
                        if (any == null) {
                            ...
                            ListenableFuture<List<BlockHeader>> futureHeaders = headersRequest.getHash() == null ?
                                    any.getCetHandler().sendGetBlockHeaders(headersRequest.getStart(), headersRequest.getCount(), headersRequest.isReverse()) :
                                    any.getCetHandler().sendGetBlockHeaders(headersRequest.getHash(), headersRequest.getCount(), headersRequest.getStep(), headersRequest.isReverse());
                            if (futureHeaders != null) {
                                Futures.addCallback(futureHeaders, new FutureCallback<List<BlockHeader>>() {
                                    ...
                                    public void onFailure(Throwable t) {
                                        ...
                                        any.getCetHandler().dropConnection();
                                    }
    ...
    private void blockRetrieveLoop() {
        class BlocksCallback implements FutureCallback<List<Block>> {
            ...
            public void onFailure(Throwable t) {
                ...
                peer.getCetHandler().dropConnection();
            }
        }
                        ...
                        for (BlockHeaderWrapper blockHeaderWrapper : bReqs.get(0).getBlockHeaders()) {
                            ...
                            if (channel != null) {
                                ListenableFuture<List<Block>> futureBlocks =
                                        channel.getCetHandler().sendGetBlockBodies(singletonList(blockHeaderWrapper));
...
//                    for (SyncQueueIfc.BlocksRequest blocksRequest : bReq.split(MAX_IN_REQUEST)) {
                        ...
                        if (any == null) {
                            ...
                            ListenableFuture<List<Block>> futureBlocks =
                                    any.getCetHandler().sendGetBlockBodies(blocksRequest.getBlockHeaders());
    ...
    /**
     * Checks whceter validation has been passed correctly or not
     ...
     * hence ancestors should call this mcetod on their own,
