...
import org.cetereum.datasource.NodeKeyCompositor;
import org.cetereum.datasource.rocksdb.RocksDbDataSource;
import org.cetereum.db.DbFlushManager;
import org.cetereum.db.HeaderStore;
import org.cetereum.db.IndexedBlockStore;
import org.cetereum.db.StateSource;
...
import org.cetereum.net.cet.handler.Cet63;
import org.cetereum.net.message.ReasonCode;
import org.cetereum.net.server.Channel;
import org.cetereum.trie.TrieKey;
import org.cetereum.util.*;
...
import static org.cetereum.listener.CetereumListener.SyncState.COMPLETE;
import static org.cetereum.listener.CetereumListener.SyncState.SECURE;
import static org.cetereum.listener.CetereumListener.SyncState.UNSECURE;
import static org.cetereum.trie.TrieKey.fromPacked;
import static org.cetereum.util.CompactEncoder.hasTerminator;
...
public class FastSyncManager {
    ...
    private static final Capability CET63_CAPABILITY = new Capability(Capability.CET, (byte) 63);
    ...
    @Autowired
    CompositeCetereumListener listener;
    ...
    public SyncStatus getSyncState() {
        ...
        CetereumListener.SyncState syncStage = getSyncStage();
        switch (syncStage) {
            case UNSECURE:
    ...
    boolean requestNextNodes(int cnt) {
        ...
        if (idle != null) {
            ...
            if (hashes.size() > 0) {
                ...
                ListenableFuture<List<Pair<byte[], byte[]>>> nodes = ((Cet63) idle.getCetHandler()).requestTrieNodes(hashes);
                    ...
                    public void onSuccess(List<Pair<byte[], byte[]>> result) {
                        try {
                            synchronized (FastSyncManager.this) {
                                ...
                                idle.getNodeStatistics().cet63NodesRequested.add(hashes.size());
                                idle.getNodeStatistics().cet63NodesRetrieveTime.add(System.currentTimeMillis() - reqTime);
                                ...
                                FastSyncManager.this.notifyAll();
                                idle.getNodeStatistics().cet63NodesReceived.add(result.size());
                            }
                        } catch (Exception e) {
...
//                idle.getCetHandler().setStatus(SyncState.IDLE);
                return false;
            }
        } else {
            return false;
    ...
    private void setSyncStage(CetereumListener.SyncState stage) {
        if (stage == null) {
    ...
    private CetereumListener.SyncState getSyncStage() {
        byte[] bytes = blockchainDB.get(FASTSYNC_DB_KEY_SYNC_STAGE);
        if (bytes == null) return UNSECURE;
        return CetereumListener.SyncState.values()[bytes[0]];
    }
...
//        }
    ...
    private void syncSecure() {
        ...
        setSyncStage(CetereumListener.SyncState.SECURE);
    ...
    private void syncBlocksReceipts() {
        ...
        if (!config.fastSyncSkipHistory()) {
            ...
            setSyncStage(CetereumListener.SyncState.COMPLETE);
    ...
    public void main() {
        ...
        if (blockchain.getBestBlock().getNumber() == 0 || getSyncStage() == SECURE || getSyncStage() == COMPLETE) {
            ...
            pool.setNodesSelector(handler -> handler.getNodeStatistics().capabilities.contains(CET63_CAPABILITY));

            try {
                CetereumListener.SyncState origSyncStage = getSyncStage();
                ...
                switch (origSyncStage) {
                    case UNSECURE:
                        ...
                        if (pivot.getNumber() == 0) {
                            ...
                            syncManager.initRegularSync(CetereumListener.SyncState.COMPLETE);
                            return;
                        }
                    ...
                    case SECURE:
                        if (origSyncStage == SECURE) {
                            ...
                            syncManager.initRegularSync(CetereumListener.SyncState.UNSECURE);
                        }
                    ...
                    case COMPLETE:
                        if (origSyncStage == COMPLETE) {
                            ...
                            logger.info("Initializing regular sync");
                            syncManager.initRegularSync(CetereumListener.SyncState.SECURE);
                        }
    ...
    private void fireSyncDone(CetereumListener.SyncState state) {
            ...
            while (true) {
                ...
                if (allIdle.size() >= MIN_PEERS_FOR_PIVOT_SELECTION || forceSyncRemains < 0 && !allIdle.isEmpty()) {
                    ...
                    for (Channel channel : allIdle) {
                        if (bestPeer.getCetHandler().getBestKnownBlock().getNumber() < channel.getCetHandler().getBestKnownBlock().getNumber()) {
                            bestPeer = channel;
                        }
                    }
                    bestKnownBlock = bestPeer.getCetHandler().getBestKnownBlock();
        ...
        if (bestIdle != null) {
            try {
                ListenableFuture<List<BlockHeader>> future =
                        bestIdle.getCetHandler().sendGetBlockHeaders(pivotBlockHash, 1, 0, false);
    ...
    private Pair<BlockHeader, Long> getPivotHeaderByNumber(long pivotBlockNumber) throws Exception {
        ...
        if (!allIdle.isEmpty()) {
            try {
                ...
                for (Channel channel : allIdle) {
                    ListenableFuture<List<BlockHeader>> future =
                            channel.getCetHandler().sendGetBlockHeaders(pivotBlockNumber, 1, false);
                    result.add(future);
                }
