...
import static org.cetereum.listener.CetereumListener.PendingTransactionState.*;
...
public class PendingStateTest {
    ...
    static class PendingListener extends CetereumListenerAdapter {
...
//        public BlockingQueue<Triple<TransactionReceipt, PendingTransactionState, Block>> onPendingTransactionUpdate = new LinkedBlockingQueue<>();
    ...
    public void testSimple() throws InterruptedException {
        ...
        bc.addCetereumListener(l);
        Triple<TransactionReceipt, CetereumListener.PendingTransactionState, Block> txUpd;
        PendingStateImpl pendingState = (PendingStateImpl) bc.getBlockchain().getPendingState();

        ECKey alice = new ECKey();

        bc.sendCeter(new byte[20], BigInteger.valueOf(100000));
        bc.sendCeter(new byte[20], BigInteger.valueOf(100000));
    ...
    public void testRebranch1() throws InterruptedException {
        ...
        PendingListener l = new PendingListener();
        bc.addCetereumListener(l);
        Triple<TransactionReceipt, CetereumListener.PendingTransactionState, Block> txUpd = null;
        PendingStateImpl pendingState = (PendingStateImpl) bc.getBlockchain().getPendingState();
        ...
        bc.sendCeter(bob.getAddress(), convert(100, CETER));
        bc.sendCeter(charlie.getAddress(), convert(100, CETER));
    ...
    public void testRebranch2() throws InterruptedException {
        ...
        bc.submitTransaction(tx1);
        bc.sendCeter(alice.getAddress(), BigInteger.valueOf(1000000));
        Block b2 = bc.createBlock();
    ...
    public void testRejected1() throws InterruptedException {
        ...
        for (int i = 0; i < 16; i++) {
            bc.createBlock();
            CetereumListener.PendingTransactionState state = l.pollTxUpdateState(tx1);
            if (state == CetereumListener.PendingTransactionState.DROPPED) {
                break;
            }
            if (i == 15) {
