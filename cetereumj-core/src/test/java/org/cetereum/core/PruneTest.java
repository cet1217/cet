...
import org.cetereum.datasource.*;
import org.cetereum.datasource.inmem.HashMapDB;
import org.cetereum.db.ByteArrayWrapper;
import org.cetereum.db.prune.Pruner;
import org.cetereum.db.prune.Segment;
...
import static org.cetereum.util.ByteUtil.intToBytes;
import static org.cetereum.util.blockchain.CeterUtil.Unit.CETER;
import static org.cetereum.util.blockchain.CeterUtil.convert;
...
//        System.out.println("Gen root: " + Hex.toHexString(bc.getBlockchain().getBestBlock().getStateRoot()));
        ...
        bc.sendCeter(alice.getAddress(), convert(3, CETER));
        ...
        bc.sendCeter(bob.getAddress(), convert(5, CETER));
        bc.createBlock();
        ...
        Assert.assertEquals(convert(30, CETER), bc.getBlockchain().getRepository().getBalance(alice.getAddress()));
        Assert.assertEquals(convert(50, CETER), bc.getBlockchain().getRepository().getBalance(bob.getAddress()));

        {
            ...
            Assert.assertEquals(convert(3 * 9, CETER), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 9, CETER), r1.getBalance(bob.getAddress()));
        }

        {
            Block b1 = bc.getBlockchain().getBlockByNumber(bestBlockNum - 2);
            Repository r1 = bc.getBlockchain().getRepository().getSnapshotTo(b1.getStateRoot());
            Assert.assertEquals(convert(3 * 8, CETER), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 8, CETER), r1.getBalance(bob.getAddress()));
        }

        {
            Block b1 = bc.getBlockchain().getBlockByNumber(bestBlockNum - 3);
            Repository r1 = bc.getBlockchain().getRepository().getSnapshotTo(b1.getStateRoot());
            Assert.assertEquals(convert(3 * 7, CETER), r1.getBalance(alice.getAddress()));
            Assert.assertEquals(convert(5 * 7, CETER), r1.getBalance(bob.getAddress()));
        }
...
//        repository.getDetailsDataStore().setStorageDS(storageDS);
    ...
    public void rewriteSameTrieNode() throws Exception {
        ...
        bc.sendCeter(receiver, BigInteger.valueOf(0x77777777));
        bc.createBlock();

        for (int i = 0; i < 100; i++) {
            bc.sendCeter(new ECKey().getAddress(), BigInteger.valueOf(i));
        }
