...
import org.cetereum.core.Bloom;
import org.cetereum.core.Transaction;
import org.cetereum.core.TransactionInfo;
...
import org.cetereum.datasource.SourceCodec;
import org.cetereum.db.BlockStore;
import org.cetereum.db.HeaderStore;
import org.cetereum.facade.Cetereum;
import org.cetereum.trie.SecureTrie;
import org.cetereum.trie.TrieImpl;
...
import static org.cetereum.core.BlockchainImpl.calcReceiptsTrie;
...
public class BlockchainValidation {
    ...
    public static void checkNodes(Cetereum cetereum, CommonConfig commonConfig, AtomicInteger fatalErrors) {
        try {
            Source<byte[], byte[]> stateDS = commonConfig.stateSource();
            byte[] stateRoot = cetereum.getBlockchain().getBestBlock().getHeader().getStateRoot();
    ...
    public static void checkHeaders(Cetereum cetereum, AtomicInteger fatalErrors) {
        int blockNumber = (int) cetereum.getBlockchain().getBestBlock().getHeader().getNumber();
        byte[] lastParentHash = null;
        testLogger.info("Checking headers from best block: {}", blockNumber);

        try {
            while (blockNumber >= 0) {
                Block currentBlock = cetereum.getBlockchain().getBlockByNumber(blockNumber);
    ...
    public static void checkFastHeaders(Cetereum cetereum, CommonConfig commonConfig, AtomicInteger fatalErrors) {
        ...
        try {
            ...
            Block genesis = cetereum.getBlockchain().getBlockByNumber(0);
    ...
    public static void checkBlocks(Cetereum cetereum, AtomicInteger fatalErrors) {
        Block currentBlock = cetereum.getBlockchain().getBestBlock();
        int blockNumber = (int) currentBlock.getHeader().getNumber();

        try {
            BlockStore blockStore = cetereum.getBlockchain().getBlockStore();
            ...
            while (blockNumber > 0) {
                currentBlock = cetereum.getBlockchain().getBlockByNumber(blockNumber);

                // Validate uncles
                assert ((BlockchainImpl) cetereum.getBlockchain()).validateUncles(currentBlock);
            ...
            currentBlock = cetereum.getBlockchain().getBlockByNumber(0);
    ...
    public static void checkTransactions(Cetereum cetereum, AtomicInteger fatalErrors) {
        int blockNumber = (int) cetereum.getBlockchain().getBestBlock().getHeader().getNumber();
        ...
        try {
            while (blockNumber > 0) {
                ...
                for (Transaction tx : currentBlock.getTransactionsList()) {
                    TransactionInfo txInfo = ((BlockchainImpl) cetereum.getBlockchain()).getTransactionInfo(tx.getHash());
                    assert txInfo != null;
    ...
    public static void fullCheck(Cetereum cetereum, CommonConfig commonConfig, AtomicInteger fatalErrors) {
