...
//        byte[] receiverAddr = Hex.decode("31e2e1ed11951c7091dfba62cd4b7145e947219c");
        ...
        for (int i = cetereum2.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 200; i++, j++) {
            {
                ...
                cetereum2.submitTransaction(tx);
...
//        });
        ...
        BlockMiner blockMiner = cetereum1.getBlockMiner();
...
//                boolean validate = Cetash.getForBlock(block.getNumber()).validate(block.getHeader());
...
//        System.out.println("======= Sending forked block without parent...");
        ...
        for (int i = cetereum1.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 20000; i++, j++) {
            {
                Transaction tx = new Transaction(ByteUtil.intToBytesNoLeadZeroes(i),
                        ByteUtil.longToBytesNoLeadZeroes(50_000_000_000L), ByteUtil.longToBytesNoLeadZeroes(0xfffff),
                        receiverAddr, new byte[]{77}, new byte[0]);
                tx.sign(senderKey);
                System.out.println("=== Submitting tx: " + tx);
                cetereum1.submitTransaction(tx);
