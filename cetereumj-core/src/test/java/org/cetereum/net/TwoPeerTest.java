...
//                return Cetash.getForBlock(header.getNumber()).mineLight(header);
//            }
//
//            @Override
...
//        });
    ...
    public void testTest() throws FileNotFoundException, InterruptedException {
        ...
        SysPropConfig1.testHandler = new Cet62() {
            @Override
        ...
        BlockchainImpl blockchain = (BlockchainImpl) cetereum1.getBlockchain();
...
//        Block b1b = addNextBlock(blockchain, bGen, "chain B");
        ...
        Cetash.getForBlock(SystemProperties.getDefault(), b1b.getNumber()).mineLight(b1b);
        Block b2b = createNextBlock(b1b, Hex.toHexString(b2.getStateRoot()), "chain B");
        Cetash.getForBlock(SystemProperties.getDefault(), b2b.getNumber()).mineLight(b2b);

        alternativeFork.add(bGen);
        alternativeFork.add(b1b);
        alternativeFork.add(b2b);

//        byte[] root = ((RepositoryImpl) cetereum.getRepository()).getRoot();
//        ((RepositoryImpl) cetereum.getRepository()).syncToRoot(root);
//        byte[] root1 = ((RepositoryImpl) cetereum.getRepository()).getRoot();
...
//        ((CetHandler) channel1[0].getCetHandler()).sendNewBlock(b2b);
