...
import org.cetereum.net.shh.MessageWatcher;
import org.cetereum.net.shh.WhisperMessage;
...
//        Thread.sleep(1000000000);
        ...
        String identity1 = cetereum1.getWhisper().newIdentity();
        String identity2 = cetereum2.getWhisper().newIdentity();

        final int[] counter1 = new int[1];
        final int[] counter2 = new int[1];

        cetereum1.getWhisper().watch(new MessageWatcher(identity1, null, null) {
            @Override
        ...
        cetereum2.getWhisper().watch(new MessageWatcher(identity2, null, null) {
            @Override
            protected void newMessage(WhisperMessage msg) {
        ...
        while (semaphoreBlocks.getCount() > 0) {
            cetereum1.getWhisper().send(identity2, "Hello Cet2!".getBytes(), null);
            cetereum2.getWhisper().send(identity1, "Hello Cet1!".getBytes(), null);
            cnt++;
