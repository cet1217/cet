 ...
 * E.g. the following is the cmd for starting up C++ cet:
 *
 * > cet --no-bootstrap --no-discovery --listen 10003 --shh --json-rpc
 *
 * If the cet is running on a remote host:port the appropriate constants in the test need to be updated
 *
...
//        remoteJsonRpc = new URL("http://whisper-1.ceter.camp:8545");
...
//                "@whisper-1.ceter.camp:30303");
        ...
        Cetereum cetereum = CetereumFactory.createCetereum(Config.class);
        cetereum.connect(
                node.getHost(),
                node.getPort(),
    ...
    public static class TestComponent extends Thread {
        ...
        void init() {
            ...
            worldManager.addListener(new CetereumListenerAdapter() {
                @Override
