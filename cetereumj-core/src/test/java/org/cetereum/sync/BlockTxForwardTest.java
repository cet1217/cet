...
import org.cetereum.core.BlockIdentifier;
...
public class BlockTxForwardTest {
    ...
    static class MinerNode extends BasicSample implements MinerListener{
        ...
        // overriding run() mcetod since we don't need to wait for any discovery,
        ...
        public void run() {
            if (config.isMineFullDataset()) {
                ...
                Cetash cetash = Cetash.getForBlock(config, cetereum.getBlockchain().getBestBlock().getNumber());
                cetash.getFullDataset();
                logger.info("Full dataset generated (loaded).");
            }
            cetereum.getBlockMiner().addListener(this);
            cetereum.getBlockMiner().startMining();
        }
    ...
    static class GeneratorNode extends BasicSample {
        ...
        private void generateTransactions() throws Exception{
            ...
            for (int i = cetereum.getRepository().getNonce(senderKey.getAddress()).intValue(), j = 0; j < 20000; i++, j++) {
                {
    ...
    /**
     *  Creating 3 CetereumJ instances with different config classes
    ...
    public void testTest() throws Exception {
        ...
        testLogger.info("Starting CetereumJ miner instance!");
        Cetereum miner = CetereumFactory.createCetereum(MinerConfig.class);

        miner.addListener(new CetereumListenerAdapter() {
            @Override
            ...
            public void onRecvMessage(Channel channel, Message message) {
                ...
                if (!(message instanceof CetMessage)) return;
                switch (((CetMessage) message).getCommand()) {
        ...
        CetereumFactory.createCetereum(RegularConfig.class);

        testLogger.info("Starting CetereumJ txSender instance!");
        Cetereum txGenerator = CetereumFactory.createCetereum(GeneratorConfig.class);
        txGenerator.addListener(new CetereumListenerAdapter() {
            @Override
            public void onRecvMessage(Channel channel, Message message) {
                super.onRecvMessage(channel, message);
                if (!(message instanceof CetMessage)) return;
            ...
            public void onSendMessage(Channel channel, Message message) {
                ...
                if (!(message instanceof CetMessage)) return;
                if (((CetMessage) message).getCommand().equals(CetMessageCodes.TRANSACTIONS)) {
                    TransactionsMessage msgCopy = new TransactionsMessage(message.getEncoded());
                    for (Transaction transaction : msgCopy.getTransactions()) {
