...
import org.cetereum.solidity.compiler.Solc;
import org.cetereum.solidity.compiler.SolidityCompiler;
import org.cetereum.util.blockchain.SolidityCallResult;
...
public class StandaloneBlockchainTest {
    ...
    public void invalidTxTest() {
        ...
        ECKey bob = new ECKey();
        sb.sendCeter(bob.getAddress(), BigInteger.valueOf(1000));
        sb.setSender(bob);
        sb.sendCeter(alice.getAddress(), BigInteger.ONE);
        sb.setSender(alice);
        sb.sendCeter(bob.getAddress(), BigInteger.valueOf(2000));
    ...
    public void initBalanceTest() {
        // check StandaloneBlockchain.withAccountBalance mcetod
        StandaloneBlockchain sb = new StandaloneBlockchain();
        ECKey alice = sb.getSender();
        ECKey bob = new ECKey();
        sb.withAccountBalance(bob.getAddress(), convert(123, CETER));
        ...
        assert convert(123, CETER).equals(bobInitBal);
        ...
        assert convert(123, CETER).compareTo(sb.getBlockchain().getRepository().getBalance(bob.getAddress())) > 0;
