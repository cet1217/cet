package org.cetereum.validator;

import org.cetereum.core.Block;
import org.cetereum.core.BlockHeader;
import org.cetereum.core.BlockSummary;
import org.cetereum.listener.CompositeCetereumListener;
import org.cetereum.listener.CetereumListener;
import org.cetereum.mine.CetashValidationHelper;
import org.junit.Test;
import org.spongycastle.util.encoders.Hex;

import java.util.Collections;

import static org.cetereum.validator.BlockHeaderRule.Success;
import static org.junit.Assert.assertEquals;
import static org.junit.Assert.assertNotEquals;
import static org.junit.Assert.assertTrue;

/**
 * @author Mikhail Kalinin
 * @since 12.07.2018
 */
public class CetashRuleTest {

    static class CompositeCetereumListenerMock extends CompositeCetereumListener {
        @Override
        public void onBlock(final BlockSummary blockSummary, final boolean best) {
            for (final CetereumListener listener : listeners) {
                listener.onBlock(blockSummary, best);
            }
        }

        @Override
        public void onSyncDone(final SyncState state) {
            for (final CetereumListener listener : listeners) {
                listener.onSyncDone(state);
            }
        }
    }

    static class CetashValidationHelperMock extends CetashValidationHelper {

        long preCacheCounter = 0;

        public CetashValidationHelperMock(CacheOrder cacheOrder) {
            super(cacheOrder);
        }

        @Override
        public void preCache(long blockNumber) {
            preCacheCounter += 1;
        }
    }

    BlockHeader validHeader = new BlockHeader(Hex.decode(""));
    BlockHeader partlyValidHeader = new BlockHeader(Hex.decode(""));
    BlockHeader invalidHeader = new BlockHeader(Hex.decode(""));

    BlockSummary dummySummaryNum_1 = new BlockSummary(new Block(validHeader, Collections.emptyList(), Collections.emptyList()),
            Collections.emptyMap(), Collections.emptyList(), Collections.emptyList());

    @Test
    public void fake() {
        System.out.println(Hex.toHexString(new Block(Hex.decode("")).getHeader().getEncoded()));
    }

    @Test
    public void testFake() {
        CompositeCetereumListener listener = new CompositeCetereumListenerMock();
        CetashRule rule = new CetashRule(CetashRule.Mode.fake, CetashRule.ChainType.main, listener);

        assertEquals(Success, rule.validate(validHeader));
        assertEquals(Success, rule.validate(partlyValidHeader));
        assertNotEquals(Success, rule.validate(invalidHeader));
    }

    @Test
    public void testStrict() {
        CompositeCetereumListener listener = new CompositeCetereumListenerMock();
        CetashRule rule = new CetashRule(CetashRule.Mode.strict, CetashRule.ChainType.main, listener);

        // cache is empty, fallback into fake rule
        assertEquals(Success, rule.validate(partlyValidHeader));

        // trigger cetash cache
        listener.onBlock(dummySummaryNum_1, true);

        assertEquals(Success, rule.validate(validHeader));
        assertNotEquals(Success, rule.validate(partlyValidHeader));
        assertNotEquals(Success, rule.validate(invalidHeader));

        // check that full verification is done on each run in strict mode
        for (int i = 0; i < 100; i++) {
            assertNotEquals(Success, rule.validate(partlyValidHeader));
        }
    }

    @Test
    public void testMixed() {
        CompositeCetereumListener listener = new CompositeCetereumListenerMock();
        CetashRule rule = new CetashRule(CetashRule.Mode.mixed, CetashRule.ChainType.main, listener);

        // trigger cetash cache
        listener.onBlock(dummySummaryNum_1, true);

        // check mixed mode randomness
        boolean fullCheckTriggered = false;
        boolean partialCheckTriggered = false;
        for (int i = 0; i < 100; i++) {
            if (Success != rule.validate(partlyValidHeader)) {
                fullCheckTriggered = true;
            } else {
                partialCheckTriggered = true;
            }

            if (partialCheckTriggered & fullCheckTriggered) break;
        }
        assertTrue(fullCheckTriggered);
        assertTrue(partialCheckTriggered);

        // trigger onSyncDone
        listener.onSyncDone(CetereumListener.SyncState.COMPLETE);

        // check that full verification is done on each run in strict mode
        for (int i = 0; i < 100; i++) {
            assertNotEquals(Success, rule.validate(partlyValidHeader));
        }
    }

    @Test
    public void testCacheMain() {
        CetashValidationHelperMock helper = new CetashValidationHelperMock(CetashValidationHelper.CacheOrder.direct);
        CompositeCetereumListener listener = new CompositeCetereumListenerMock();
        CetashRule rule = new CetashRule(CetashRule.Mode.mixed, CetashRule.ChainType.main, listener);
        rule.cetashHelper = helper;

        // trigger cache
        for (int i = 0; i < 100; i++) {
            listener.onBlock(dummySummaryNum_1, false);
            listener.onBlock(dummySummaryNum_1, true);
        }

        // must be triggered on best block only
        assertEquals(100, helper.preCacheCounter);
    }

    @Test
    public void testCacheDirect() {
        CetashValidationHelperMock helper = new CetashValidationHelperMock(CetashValidationHelper.CacheOrder.direct);
        CompositeCetereumListener listener = new CompositeCetereumListenerMock();
        CetashRule rule = new CetashRule(CetashRule.Mode.mixed, CetashRule.ChainType.direct, listener);
        rule.cetashHelper = helper;

        // trigger cache
        for (int i = 0; i < 100; i++) {
            rule.validate(validHeader);
        }

        // must be triggered each verification attempt, in spite of mixed mode
        assertEquals(100, helper.preCacheCounter);
    }
    @Test
    public void testCacheReverse() {
        CetashValidationHelperMock helper = new CetashValidationHelperMock(CetashValidationHelper.CacheOrder.direct);
        CompositeCetereumListener listener = new CompositeCetereumListenerMock();
        CetashRule rule = new CetashRule(CetashRule.Mode.mixed, CetashRule.ChainType.reverse, listener);
        rule.cetashHelper = helper;

        // trigger cache
        for (int i = 0; i < 100; i++) {
            rule.validate(validHeader);
        }

        // must be triggered each verification attempt, in spite of mixed mode
        assertEquals(100, helper.preCacheCounter);
    }
}
